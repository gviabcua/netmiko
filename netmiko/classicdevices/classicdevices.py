...
class ClassicdevicesBase(CiscoBaseConnection):
    def session_preparation(self) -> None:
        """Prepare the session after the connection has been established."""
        self._test_channel_read(pattern=r"[>#]")
        self.set_base_prompt()
        self.enable()
        self.disable_paging("terminal length 0")
...
class ClassicdevicesSSH(ClassicdevicesBase):
    def special_login_handler(self, delay_factor: float = 1.0) -> None:
        """
        Classicdevices presents with the following on login (in certain OS versions)
        Login: user
        Password:****
        """
...
class ClassicdevicesTelnet(ClassicdevicesBase):
    @staticmethod
